...
const appState = {
    ...
    isDarkMode: true,
    isAdmin: false,
    adminEmail: 'admin@questio.com', // Ganti dengan email admin Anda
...
function initEventListeners() {
    ...
    document.getElementById('questionClose').addEventListener('click', hideQuestionModal);
    document.getElementById('addQuestionClose').addEventListener('click', hideAddQuestionModal);
    
    // Close modals on overlay click
    [elements.authModal, elements.questionModal, elements.addQuestionModal].forEach(modal => {
...
async function handleAuth(e) {
    ...
    try {
        ...
        hideLoadingBtn(elements.authBtn, elements.authLoading, elements.authBtnText);
    }
}
...
function toggleAuthMode() {
    ...
    elements.modalTitle.textContent = isLoginMode ? 'Login ke Questio' : 'Daftar ke Questio';
...
function showQuestionModal(question) {
    ...
    document.getElementById('questionDifficulty').textContent = question.tingkatKesulitan;
    document.getElementById('questionTime').textContent = `${question.waktuPengerjaan} menit`;
...
function showEmptyState() {
    elements.questionsGrid.innerHTML = `
        <div class="empty-state">
            ...
            <p>${appState.isAdmin ? 'Tambahkan soal pertama Anda!' : 'Soal akan muncul di sini setelah ada yang mengunggah.'}</p>
        </div>
    `;
}
...
async function loadQuestions() {
    showSkeleton();
    try {
        const snapshot = await db.collection('questions').orderBy('tanggalUpload', 'desc').get();
...
function handleFilter(filter) {
    ...
    if (filter === 'all') {
        ...
        appState.filteredQuestions = appState.questions.filter(q => 
            q.mapel.toLowerCase() === filter.toLowerCase()
        );
    }
...
function renderQuestions() {
    ...
    elements.questionsGrid.innerHTML = appState.filteredQuestions.map(question => createQuestionCard(question)).join('');
    
    // Add click listeners to cards
    document.querySelectorAll('.question-card:not(.skeleton)').forEach(card => {
        card.addEventListener('click', () => showQuestionModal(questionFromCard(card)));
    });
}
...
function createQuestionCard(question) {
    ...
    return `
        <div class="question-card" data-id="${question.id}">
            ${question.gambarSoal ? `<img src="${question.gambarSoal}" alt="${question.judul}" class="question-image">` : ''}
            <div class="question-title">${question.judul}</div>
            <div class="question-meta">
                <span class="meta-tag subject">${question.mapel}</span>
                <span class="meta-tag difficulty" style="color: ${difficultyColor}">${question.tingkatKesulitan}</span>
...
async function handleAddQuestion(e) {
    ...
    showLoadingBtn(document.querySelector('#questionForm .btn-primary'), 
    ...
    try {
        ...
        hideLoadingBtn(document.querySelector('#questionForm .btn-primary'), 
                       document.getElementById('submitLoading'), 
                       document.getElementById('submitBtnText'));
    }
}
...
async function toggleFavorite() {
    ...
    try {
        const favoriteRef = db.collection('favorites').doc(`${appState.currentUser.uid}_${appState.currentQuestion.id}`);
